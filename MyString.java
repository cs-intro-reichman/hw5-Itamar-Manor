...
public class MyString {
     ...
     * letter can be selected more than once.
     * <p>
     * If the moreVowels parameter is true, then doubles the bias towards vowels.
     * 
     * @param n          - the number of letter to select
     * @param moreVowels - whether to add bias to vowels
     * @return a randomly generated string, consisting of 'n' lowercase letters
     */
    public static String randomStringOfLetters(int n, boolean moreVowels) {
        if (!moreVowels) {
            return randomStringOfLetters(n);
        }
        String newStr = "";
        String vowels = "aeiou";
        for (int i = 0; i < n; i++) {
            char c;
            int idk = rand('a', 'z' + 1 + vowels.length());
            if (idk > 'z') {
                c = vowels.charAt(rand(0, vowels.length()));
            } else {
                c = (char) idk;
            }
            newStr += c;
