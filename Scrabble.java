...
public class Scrabble {
	...
	public static boolean isWordInDictionary(String word) {
		int l = 0;
		int h = NUM_OF_WORDS - 1;
		int g = (l + h) / 2;

		while (!word.equals(DICTIONARY[g]) && l != g && h != g) {
			if (word.compareTo(DICTIONARY[g]) > 0) {
				l = g;
			} else {
				h = g;
			}
		...
		return word.equals(DICTIONARY[g]);
	}
	...
	public static int wordScore(String word) {
		...
		if (MyString.subsetOf("runi", word)) { // This is dumb :-9
			score += 1000;
		}
		return score;
	}
	...
	public static String createHand() {
		...
		int a_index = (int) (Math.random() * hand.length());
		hand = hand.substring(0, a_index + 1) + "a" + hand.substring(a_index + 1, hand.length());
		int e_index = (int) (Math.random() * hand.length());
		hand = hand.substring(0, e_index + 1) + "e" + hand.substring(e_index + 1, hand.length());
